...
def hit_or_miss(set: Set, se: StructuralElement):
    ...
    if not se.set.point_in_bbox(se.origin):
        raise ValueError(
    ...
    for i in np.ndindex(set.size.tuple()):
        origin = subtract_tuples(i, (se.origin.y, se.origin.x))
        ...
        result.object_mask[i] = np.all(
            cutout == se.set.clipped(Point(origin[0], origin[1]), set.size).object_mask
        )
    return result
